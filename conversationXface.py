...
# this function will initiate the recognize service and pass in the AudioSource
    def recognize_using_weboscket(self, *args):
        ...
        remove_word_list             = self.config["Transformations"]["remove_word_list"]

        lower_case                   = self.str2bool(self.config["Transformations"]["lower_case"])
        remove_punctuation           = self.str2bool(self.config["Transformations"]["remove_punctuation"])
        remove_multiple_spaces       = self.str2bool(self.config["Transformations"]["remove_multiple_spaces"])
        remove_white_space           = self.str2bool(self.config["Transformations"]["remove_white_space"])
        sentences_to_words           = self.str2bool(self.config["Transformations"]["sentences_to_words"])
        strip                        = self.str2bool(self.config["Transformations"]["strip"])
        remove_empty_strings         = self.str2bool(self.config["Transformations"]["remove_empty_strings"])
...
def delayed_call_function_stop_stream():
    global stream_started
    global recognize_thread
    stream.stop_stream()
    stream_started = False
    #Not sure how to stop this thread without this. The websocket doesn't require a loop to stay open so we cannot stop it with an event. We can't just leave the threads going.
    recognize_thread.kill()
    

def face_callback(data):
    global stream_started
    global recognize_thread
    if (len(data.data) > 0 and stream_started == False):
        stream_started = True
        stream.start_stream()
        Timer(60.0, delayed_call_function_stop_stream).start()
...
def main(args):
    ...
    try:
        ...
        node_thread.start()
        sub_thread = Thread(target=rospy.Subscriber(args.face_topic, String, face_callback))
        sub_thread.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--face_topic', '-f', default='/face_detections', help='The topic that provides a list of faces detected from your robots camera. Default is /face_detections from the hri_pack')
    parser.add_argument('--tts', help='If your robot has a built in speech synthesiser, provide the topic that is needed to publish to it. example: python conversation.py --tts /speech/say. This only publishes a string in the format { "data": "Speech goes here" }. If you need a different format you will have to edit this file.')
